...
for factor in range(x, 0, -1):
    if x % factor == 0 and y % factor == 0:
        ...
        #与最小公倍数相对应的概念是最大公约数，a，b的最大公约数记为（a，b）。a，b的最小公倍数记为[a,b]。关于最小公倍数与最大公约数，我们有这样的定理：(a,b)x[a,b]=ab(a,b均为整数)。
        break
```
...
row = int(input('请输入行数: '))
for i in range(row):        # range(5)等价于range(0,5),也就是从0开始。range函数是左包右不包。
    for _ in range(i + 1):
        print('*', end='')
    print()
